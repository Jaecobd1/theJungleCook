RECIPES = [{
        ...
        instructions: [{
            value: 'Preheat the oven to 475°. Spray pizza pan with nonstick cooking or line a baking sheet with parchment paper.'
        }, {
            ...
            value: 'Bake for 8 to 10 minutes or until the crust is crisp and the cheese melted and lightly browned.'
        }]
    },
    {
        name: "Classic Burger",
...
function createAccount() {
    ...
    console.log("Create " + fullName + email + password)
...
function initFirebase() {
    ...
    firebase.auth().onAuthStateChanged((user) => {
        if (user) {
            ...
            _db.collection('Recipes').get().then((recipe) => {
                recipe.forEach((doc) => {
...
function loadRecipeItem(idx) {
    ...
    if (uid == recipes[idx].creator) {
        $("#edit").html(`<div class="editBtn" onclick="editRecipe(${idx})" > Edit </div>`)
    } else {
        $("#edit").empty();
    }
}
